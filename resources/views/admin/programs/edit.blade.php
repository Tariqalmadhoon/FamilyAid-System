<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit: {{ $program->name }}</h2>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('admin.programs.update', $program) }}">
                    ...
                    <div class="flex justify-between">
                        <form action="{{ route('admin.programs.destroy', $program) }}" method="POST" onsubmit="return confirm('Delete this program?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="px-4 py-2 text-red-600 hover:text-red-800 text-sm">Delete</button>
                        </form>
                        <div class="flex space-x-3">
                            ...
                            <button type="submit" class="px-4 py-2 bg-teal-600 text-white rounded-md text-sm font-medium hover:bg-teal-700">Save</button>
                        </div>
                    </div>
                </form>
            </div>
