            ...
            <div class="bg-white rounded-lg shadow-sm">
                <form method="POST" action="{{ route('admin.households.store') }}" class="p-6">
                    ...
                    <div class="mt-4 border-t border-gray-200 pt-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="spouse_full_name" value="{{ old('spouse_full_name') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500" required>
                            ...
                            <div>
                                ...
                                <input type="tel" name="spouse_national_id" value="{{ old('spouse_national_id') }}" maxlength="9" inputmode="numeric" oninput="this.value=this.value.replace(/\\D/g,'').slice(0,9)" class="w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500" required>
                            ...
                            <div>
                                ...
                                <input type="date" name="spouse_birth_date" value="{{ old('spouse_birth_date') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500" required>
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-200">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                                    <input type="checkbox" name="spouse_has_war_injury" value="1" {{ old('spouse_has_war_injury') ? 'checked' : '' }} class="rounded border-gray-300 text-teal-600 focus:ring-teal-500">
                                    <span>{{ __('messages.health.war_injury') }}</span>
                                </label>
                                <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                                    <input type="checkbox" name="spouse_has_chronic_disease" value="1" {{ old('spouse_has_chronic_disease') ? 'checked' : '' }} class="rounded border-gray-300 text-teal-600 focus:ring-teal-500">
                                    <span>{{ __('messages.health.chronic_disease') }}</span>
                                </label>
                                <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                                    <input type="checkbox" name="spouse_has_disability" value="1" {{ old('spouse_has_disability') ? 'checked' : '' }} class="rounded border-gray-300 text-teal-600 focus:ring-teal-500">
                            ...
                            <div class="mt-3">
                                ...
                                <input type="text" name="spouse_condition_type" value="{{ old('spouse_condition_type') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 text-sm" placeholder="{{ __('messages.onboarding_form.spouse_condition_type_placeholder') }}">
                            ...
                            <div class="mt-3">
                                ...
                                <textarea name="spouse_health_notes" rows="2" placeholder="{{ __('messages.onboarding_form.spouse_health_notes_placeholder') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 text-sm">{{ old('spouse_health_notes') }}</textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <select name="housing_type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500">
                                ...
                                @foreach($housingTypes as $type)
                                    <option value="{{ $type }}" {{ old('housing_type') === $type ? 'selected' : '' }}>{{ __('messages.housing_types.' . $type) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="status" class="w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500" required>
                                <option value="pending" {{ old('status', 'pending') === 'pending' ? 'selected' : '' }}>{{ __('messages.status.pending') }}</option>
                                <option value="verified" {{ old('status') === 'verified' ? 'selected' : '' }}>{{ __('messages.status.verified') }}</option>
                                <option value="suspended" {{ old('status') === 'suspended' ? 'selected' : '' }}>{{ __('messages.status.suspended') }}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        <a href="{{ route('admin.households.index') }}" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">{{ __('messages.actions.cancel') }}</a>
                        <button type="submit" class="px-4 py-2 bg-teal-600 text-white rounded-md text-sm font-medium hover:bg-teal-700">{{ __('Create Household') }}</button>
                    </div>
                </form>
            </div>
        </div>
